...
if [[ $TRAVIS_REPO_SLUG == "deveth0/httpclient-spring-boot-starter" ]] && [[ "$TRAVIS_PULL_REQUEST" == "false" ]] && [[ "$TRAVIS_BRANCH" == "travis_sonatype" || "$TRAVIS_BRANCH" == "develop" ]]; then
  if [[ $TRAVIS_JDK_VERSION == "openjdk11" ]]; then
    ...
    echo -e "Successfully imported GPG keys, listing keys"
    gpg --list-secret-keys $TRAVIS_BUILD_DIR/secring.gpg
